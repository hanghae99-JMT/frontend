...
const SignUp = () => {
  ...
  const checkVal = () => {
    ...
    if (input_pw.current.value.length < 6 || !pwRegExp.test(input_pw.current.value)) {
      res = false;
      new_errors[2] = true;
    } else {
      new_errors[2] = false;
    }
    if (input_pw_confirm.current.value.length < 6 || (input_pw.current.value != input_pw_confirm.current.value)) {
      res = false;
      new_errors[3] = true;
    } else {
      new_errors[3] = false;
    ...
    <>
      <Box
        style={{
          ...
          backgroundImage: "url('https://images.unsplash.com/photo-1657299156537-f4bcdced5392?ixlib=rb-1.2.1&ixid=MnwxMjA3fDF8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=2670&q=80')",
      ...
      >
        <Box sx={{backgroundColor: "rgba(255,255,255,0.8)", padding: "2em", boxShadow: "0 3px 8px rgba(0,0,0,.5)", borderRadius: "5px"}}>
          ...
          <UserTextField
            ...
            onChange={checkVal}
            error={errors[2]}
            helperText={errors[2] ? "영문자와 특수문자(!@#$%^&*)포함\n 8-16자로 입력해주세요."  : false}
          />
          <UserTextField
